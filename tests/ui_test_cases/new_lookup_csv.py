...
class NewLookupCsv(unittest.TestCase):
    def setUp(self):
    ...
    def test_new_lookup_csv(self):
        ...
        driver.find_element_by_link_text("Create CSV Lookup...").click()
        for i in range(60):
            try:
        ...
        print(base_url)
        self.assertRegexpMatches(driver.current_url, r"^\$\{base_url\}[\s\S]action=new&type=csv$")
        for i in range(60):
            try:
                if 1 == len(driver.find_elements_by_css_selector("#lookup-table")): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
        self.assertEqual(1, len(driver.find_elements_by_css_selector("#lookup-table")))
